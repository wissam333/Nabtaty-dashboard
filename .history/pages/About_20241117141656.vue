          ...
          <tbody>
            <tr class="rows" v-for="cat in About" :key="cat.id">
              <td>
                <div>
                  <span>
                    <img height="50" :src="apiBase + '/' + cat.photo" alt="" />
                  </span>
                </div>
              </td>
              <td>
...
const DeleteCat = async () => {
  const { RemovePlant, error } = await useFetch(
    `${api.DeleteAboutInfo}/${deleteRegion.value.id}`,
    {
      baseURL: apiBase,
      method: "DELETE",
    }
...
const addInfo = async () => {
  const { data } = await useFetch(api.AddAboutInfo, {
    baseURL: apiBase,
    method: "POST",
    body: {
      phone: phone.value,
...
const editCat = async () => {
  const { edit, error } = await useFetch(
    `${api.EditAboutInfo}/${editedCat.value.id}`,
    {
      body: {
