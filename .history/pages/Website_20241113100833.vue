...
watchEffect(() => {
  if (process.client) {
    if (MainImg.value) {
      ...
      src.value.id = apiBase + "/" + MainImg.value[0].id;
      src1.value.id = apiBase + "/" + MainImg.value[1].id;
      src2.value.id = apiBase + "/" + MainImg.value[2].id;
      src3.value.id = apiBase + "/" + MainImg.value[3].id;
    }
  }
});

const removeImage = async (src) => {
  const { RemoveHomeImg, error } = await useFetch(`${api.RemoveHomeImg}`, {
    baseURL: apiBase,
    method: "POST",
  });
};
