...
const editfunc = async (info) => {
  try {
    ...
    editDescription_AR.value = ar.value.editDescription_AR || "";
    editDescription_EN.value = en.value.editDescription_EN || "";
    editsrc.value = apiBase + "/" + en.value.photo;

    // Fetch the image as a Blob
    const response = await fetch(editsrc.value);
    if (!response.ok) throw new Error("Failed to fetch image.");
    editsrcFile.value = await response.blob();
  } catch (error) {
    console.error("Error in editfunc:", error);
    $awn.error(locale.value === "en" ? "Error fetching data" : "خطأ في جلب البيانات", {
      durations: { global: 5000 },
    });
  }
};
...
const editCat = async () => {
  try {
    ...
    const { data: edit, error } = await useFetch(
    ...
    console.error("Error in editCat:", error);
    $awn.error(locale.value === "en" ? "Error updating category" : "خطأ في تعديل الفئة", {
      durations: { global: 5000 },
    });
  }
};
