<template>
  <div class="mb-5 position-relative">    
    <VField :name="name" v-slot="{ field, meta }">
      <label class="fw-bold w-100 text-start" :for="id" :style="`color: #${color}`" :class="`form-label mb-3   ${$i18n.locale == 'ar' ? 'rtl' : '' }`" v-if="label">
      {{ label }} 
    ...
    <label :for="id" :class="`form-label text-white ${$i18n.locale == 'ar' ? 'rtl' : '' }`" v-if="label">{{ label }}</label>
  </div>
</template>
